""" Attention world model training """
import argparse
...
from models.mdrnn import MDATTN, gmm_loss

parser = argparse.ArgumentParser("MDATTN training")
...
mdrnn = MDATTN(LSIZE, ASIZE, RSIZE, 5)
mdrnn.to(device)
...
if exists(rnn_file) and not args.noreload:
    ...
    state_dict = rnn_state["state_dict"]
    is_attn_checkpoint = any(
        key.startswith('encoder.') or key.startswith('input_proj.')
        or key.startswith('pos_embedding')
        for key in state_dict)

    if is_attn_checkpoint:
        print("Loading MDATTN at epoch {} "
              "with test error {}".format(
                  rnn_state["epoch"], rnn_state["precision"]))
        mdrnn.load_state_dict(state_dict)
        optimizer.load_state_dict(rnn_state["optimizer"])
        scheduler.load_state_dict(rnn_state['scheduler'])
        earlystopping.load_state_dict(rnn_state['earlystopping'])
    else:
        print("Found legacy MDRNN checkpoint at {}. "
              "Skipping reload and training MDATTN from scratch.".format(rnn_file))
